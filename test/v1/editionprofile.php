...
$bdd = new PDO('mysql:host=127.0.0.1;dbname=omega_db', 'root','');
...
if(isset($_SESSION['Pseudo']) )
{
    ...
    $requser = $bdd->prepare("SELECT * FROM omega_membre WHERE Pseudo = ?");
    ...
    if((isset($_POST['newmail']) AND !empty($_POST['newmail']) AND $_POST['newmail'] != $user['Email']) )
    {
        ...
        if(filter_var($newmail, FILTER_VALIDATE_EMAIL))
        {
            ...
            $reqmail = $bdd->prepare("SELECT * FROM omega_membre WHERE Email = ? AND Pseudo != ? ");
            ...
            if($mailexist == 0)
            {
                ...
                $updateemail = $bdd->prepare("UPDATE omega_membre SET Email=? WHERE Pseudo = ?");
    ...
    if (isset($_POST['newmdp']) AND !empty($_POST['newmdp']) AND $_POST['newmdp'] != $user['Pass']) 
    {
        ...
        {
            //Mettre à jour la BDD avec les nouvelles informations
            $updatemdp = $bdd->prepare("UPDATE omega_membre SET Pass=? WHERE Pseudo = ?");
